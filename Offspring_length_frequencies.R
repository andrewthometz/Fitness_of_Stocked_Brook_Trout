...
#### Create length-frequency distributions for each wild cohort ####
Offspring_2111 <- read_delim("X:/2111_F1F2D_BKT/2111analysis/Thometz_scripts/Samples_2111.csv") %>% 
  ...
  mutate(Capture = case_when(Capture_date == "Sep-8-2020" ~ "Fall 2020",
                             ...
                             Capture_date == "Oct-6-2022" ~ "Fall 2022"))

Offspring_2111 %>% count(Capture_date)
Offspring_2111 %>% count(Cohort)

# Offspring from W_2020 cohort (Don't have length data for fish captured on 4/5/2020 (n = 28)) Doesn't matter...
plot_2019 <- Offspring_2111 %>% 
  filter(Capture_date == "Sep-8-2020") %>% # Don't have length data for Apr-5-2021 (n = 28 fish)
